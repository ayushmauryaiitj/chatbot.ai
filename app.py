...
SCHEMES = [
    {
        ...
        "benefit": "6000 rupees per year in 3 installments",
        "benefit_hi": "साल में 6000 रुपये तीन किस्तों में",
    ...
    },
    {
        ...
        "benefit": "Free skill training and job placement assistance",
    ...
    },
    {
        ...
        "eligibility": "Families with annual income less than 1 lakh rupees",
        "eligibility_hi": "साल में 1 लाख से कम कमाने वाले परिवार",
        "benefit": "Free health coverage up to 5 lakh rupees per family per year",
        "benefit_hi": "प्रति परिवार प्रति वर्ष 5 लाख रुपये तक मुफ्त स्वास्थ्य कवरेज",
...
def answer_question(query):
    ...
    if any(word in query_lower for word in ["eligibility", "पात्रता", "eligible", "qualify", "कौन", "किसके लिए"]):
        ...
        return tr(
            f"For **{name}**, eligibility is: {elig}",
            f"**{name}** के लिए पात्रता है: {elig}"
        ), scheme
    
    elif any(word in query_lower for word in ["benefit", "लाभ", "मिलता", "मिलेगा", "get", "फायदा"]):
        scheme = find_schemes(query)[0]
        name = scheme["name_hi"] if st.session_state.lang == "हिंदी" else scheme["name"]
        benefit = scheme["benefit_hi"] if st.session_state.lang == "हिंदी" else scheme["benefit"]
        return tr(
            f"**{name}** provides: {benefit}",
            f"**{name}** में मिलता है: {benefit}"
        ), scheme
    
    elif any(word in query_lower for word in ["apply", "आवेदन", "कैसे", "how", "process"]):
        scheme = find_schemes(query)[0]
        name = scheme["name_hi"] if st.session_state.lang == "हिंदी" else scheme["name"]
        how = scheme["how_to_apply_hi"] if st.session_state.lang == "हिंदी" else scheme["how_to_apply"]
        return tr(
            f"To apply for **{name}**: {how}",
            f"**{name}** के लिए आवेदन: {how}"
        ), scheme
...
st.markdown("""
<style>
.stApp { background: linear-gradient(135deg, #04111d 0%, #0a1f33 100%); }
</style>
    ...
    with st.chat_message("assistant"):
        with st.spinner(tr("Thinking...", "सोच रहा हूं...")):
            ...
            if answer:
                ...
                msg = tr(
                    f"I found **{len(scheme_data)} schemes** that might help you:",
                    f"मैंने **{len(scheme_data)} योजनाएं** ढूंढी जो आपकी मदद कर सकती हैं:"
                )
                schemes_list = ""
